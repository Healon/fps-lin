...
export type DoorOpenCondition = "has-weapon" | "area-clear:B" | "area-clear:C";
...
export function generateLevel(): LevelData {
  ...
  // ============================================================
  // 終點小室：終點門後的收尾空間，走入 endTrigger 即觸發通關畫面。X:[44,48] Z:[-12,-8]。
  // ============================================================
  ...
  addSolid(wallTarget, colliders, b(48 + WALL_T, H_TALL / 2, -10, WALL_T, H_TALL / 2, 2 + WALL_T));

  const endTrigger: Aabb = {
    min: { x: 44.5, y: 0, z: -11.5 },
    max: { x: 48, y: 2.2, z: -8.5 },
  };

  // ---- 匯總 ----
  const doors: DoorDef[] = [doorA, doorB, endDoor];
  const pickups: PickupDef[] = [pistolPickup, ...pickupsB, shotgunPickup, ...pickupsC];
  const enemySpawns: EnemySpawnDef[] = [...enemiesB, ...enemiesC];

  const levelHash = computeLevelHash(colliders, doors, pickups, enemySpawns, endTrigger, playerSpawn);
